...
<script lang="ts">
  ...
  const submit = async (e) => {
    ...
    if (timingLookup) {
      console.debug('Will export qualifs with timing filters :', timingLookup);
      formData.append(`timingLookup`, JSON.stringify(timingLookup));
    }
    const formJson = Object.fromEntries(formData.entries());
    console.log('Should export qualifs : ', formJson);

    var file_path = Routing.generate("mws_timing_qualif_export", {
        _locale: locale,
        ...formJson,
    }),
...
<form on:submit|preventDefault={submit} class="mws-export-qualifs-form">
  <select
  bind:value={format}
  name="format"
