<script lang="ts">
  ...
  export let importTags = async (
    ...
    const data = {
      _csrf_token: stateGet(get(state), "csrfQualifImport"),
    ...
    const resp = await fetch(
      ...
      Routing.generate("mws_timing_qualif_import", {
        _locale: locale,
      }),
      {
        method: "POST",
      ...
      .then(async (resp) => {
        ...
        if (!resp.ok) {
          ...
          // qualifs = Object.values(data.newTags); // A stringified obj with '1' as index...
          // TODO : force reload, but should recompute ?
          // importedTags = data.qualifs;
          // importedTagsGrouped = data.qualifsGrouped;
          importReport = data.importReport;
          reportModal.eltModal.show();
          stateUpdate(state, {
            csrfQualifImport: data.newCsrf,
          });
        }
      })
      .catch((e) => {
  ...
  const submit = async (e) => {
    ...
    console.log("Should import qualifs : ", formJson);
...
<form on:submit|preventDefault={submit} class="mws-import-qualifs-form">
  {#if format}
    <input
      type="file"
      name="importFile"
